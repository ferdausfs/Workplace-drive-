...
async function getMatchingSubscribers(signal, env) {
...
async function claimPushLock(chatId, signal, env) {
  ...
  const key = PUSH_LOCK_PREFIX + chatId + ":" + normPair(signal.pair) + ":" + signal.signal.finalSignal;
  try {
...
async function pushSignalToSubscribers(signal, env) {
  ...
  try {
    ...
    console.log("Phase 10 push: " + signal.id + " " + signal.pair + " " + sig.finalSignal + " -> " + delivered.length + "/" + eligible.length + " delivered (" + subscribers.length + " matched)");
...
async function getPushStats(env) {
...
function getSignalGrade(confidence, avgConf, alignment, structureOverall) {
  let sc = 0;
...
async function handleHealth(env) {
  ...
  const phase10 = await getPushStats(env);
  return jsonResponse({
    status: "healthy",
    version: "6.9.2",
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
...
function idxKey(pair) {
  return IDX_PREFIX + pairKey2(pair);
}
...
function idxKey2(pair) {
  return IDX_PREFIX2 + pairKey3(pair);
}
...
async function buildMultiTimeframeSignal(pair, candleData, assetType, env, opts = {}) {
  ...
  const finalGrade = finalDirection === "NO_TRADE" ? { grade: "N/A", label: "NO_TRADE", description: "Engine blocked \u2014 no trade." } : getSignalGrade(confidence, avgConf, alignment, structureVerdict.overall);
  const __signal = {
...
function shadowIdxKey(pair) {
  return IDX_PREFIX3 + pairKey4(pair);
}
...
async function scheduledScan(env, ctx) {
